...
<!--                    </template>-->
                    ...
                    <v-card
                        ...
                        text="You are going to delete this genre"
                        title="Are You Sure?"
                    >
...
const editItem = function (item) {
    router.visit('/genres/' + item.id + '/edit')
}
...
const destroy=(item)=>{
    ...
    axios.delete('/genres-api/'+item.id).then(response => {
...
const loadItems = function ({page, itemsPerPage, sortBy}) {
    ...
    axios.get('/genres-api').then(response => {
        serverItems.value = response.data.data
        totalItems.value = response.data.meta.total
    }).finally(() => {
        loading.value = false
